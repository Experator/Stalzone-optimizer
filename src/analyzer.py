...
def get_default_toggles(report: HardwareReport) -> List[OptimizationToggle]:
    ...
    return [
        ...
        # SYSTEM
        t("disable_usb_power_saving", "system", "Отключить энергосбережение USB",
          "USB selective suspend off. Предотвращает отключения мыши/клавиатуры/наушников.",
          True, True, "medium", True, True),
        t("disable_pci_express_link_state", "system", "Отключить PCIe ASPM",
          "Link State Power Management off. Стабильность GPU, без просадок частоты.",
          True, True, "medium", True, True),
        t("clear_font_cache", "system", "Очистить кэш шрифтов",
          "Перезапуск FontCache + удаление .dat файлов. Ускоряет отрисовку UI.",
          False, False, "low", True, True),
        t("disable_windows_defender_realtime", "system", "Отключить Defender RT (временно)",
          "ВНИМАНИЕ: отключает real-time protection на время игры. Включите обратно после!",
          False, False, "high", True, True),
        t("disable_windows_update", "system", "Отключить Windows Update (временно)",
          "Останавливает wuauserv + UsoSvc. Предотвращает фоновые загрузки во время игры.",
          False, False, "medium", True, True),
        t("clear_temp_files", "system", "Очистить временные файлы",
          "Удаляет файлы из TEMP и C:\\Windows\\Temp. Снижает I/O нагрузку.",
          False, False, "low", False, True),
        t("disable_notifications", "system", "Отключить уведомления (Focus Assist)",
          "Тихий режим — уведомления не всплывают во время игры.",
          True, True, "low", False, True),
        t("set_process_priority_class", "system", "AppCompat: высокий приоритет для игр",
          "Registry-флаг для совместимости — Windows даёт играм высокий приоритет.",
          False, False, "low", True, True),
        t("disable_background_apps_global", "system", "Глобально отключить фоновые приложения",
          "Запрещает UWP-приложениям работать в фоне. Экономит CPU/RAM.",
          is_low or is_mid, is_low, "medium", True, True),
        t("disable_cortana", "system", "Отключить Cortana",
          "Отключает голосовой помощник. Освобождает CPU/RAM.",
          True, True, "low", True, True),
    ]
